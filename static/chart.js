...
async function loadRecentScans() {
  ...
  try {
    ...
    data.forEach((d, i) => {
      ...
      const issues =
        (d.outdated_dates?.length || 0) +
        (d.broken_links?.length || 0) +
        (d.invalid_contacts?.length || 0) +
      ...
      html += `
        ...
        >
          ...
          <div class="scan-item-right">
            <span class="scan-severity ${severityClass}">
              ${severity}
            </span>
...
async function drawOverviewChart() {
  try {
    ...
    const labels = data.map(d =>
      d.scanned_on
        ? d.scanned_on.substr(0, 10)
        : 'unknown'
    );

    const outdatedValues = data.map(d =>
      Array.isArray(d.outdated_dates)
        ? d.outdated_dates.length
        : 0
    );

    const brokenValues = data.map(d =>
      Array.isArray(d.broken_links)
        ? d.broken_links.length
        : 0
    );

    const contactValues = data.map(d =>
      Array.isArray(d.invalid_contacts)
        ? d.invalid_contacts.length
        : 0
    );
...
async function loadAnalyticsSummary() {
  try {
    ...
    const totalOutdated = data.reduce(
      (sum, d) => sum + (d.outdated_dates?.length || 0),
      0
    );

    const totalBroken = data.reduce(
      (sum, d) => sum + (d.broken_links?.length || 0),
      0
    );

    const totalInvalid = data.reduce(
      (sum, d) => sum + (d.invalid_contacts?.length || 0),
      0
    );

    const avgIssues = Math.round(
      (totalOutdated + totalBroken + totalInvalid) / totalScans
    );
        ...
        <div class="analytics-grid">
          ...
          <div class="analytics-card">
            ...
            <div class="analytics-percent">
              ${((healthyScans / totalScans) * 100).toFixed(0)}%
            </div>
          </div>
          ...
          <div class="analytics-card">
            ...
            <div class="analytics-value">
              ${totalOutdated + totalBroken + totalInvalid}
            </div>
          </div>
